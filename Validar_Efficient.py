# -*- coding: utf-8 -*-
"""
Script para validar un modelo CNN ya entrenado, cargando primero la
arquitectura y luego los pesos guardados (.h5).
"""
...
from tensorflow.keras.layers import Input, Convolution2D, Flatten, Dense, Dropout
from tensorflow.keras.applications import EfficientNetB0 # <- Importa la arquitectura que usaste
...
data_dir_validacion = 'E:/Angie/Experimento3_Validacion' 
# Ruta donde guardaste los PESOS de tu modelo (archivo .h5)
# Asegúrate de especificar qué pliegue quieres validar, ej. el pliegue 1
ruta_pesos_h5 = 'E:/Angie/codigos/Modelos_Guardados/pesos_pliegue_2.h5'
# Número de bandas del experimento que estás validando
NUM_BANDAS = 1

# --- 3. RE-CREACIÓN DE LA ARQUITECTURA DEL MODELO ---
# Esta función debe ser IDÉNTICA a la del script de entrenamiento.
def crear_arquitectura_efficientnetb0(num_bandas, altura=214, longitud=214):
    """
    Crea un modelo basado en la arquitectura EfficientNetB0.
    """
...
# --- 4. Preparación de los Datos de Validación ---
...
def generador_multibanda(dataframe, batch_size, num_bandas, shuffle=False, target_size=(214, 214)):
    ...
    while True:
        ...
        for offset in range(0, num_muestras, batch_size):
            ...
            yield X_lote, y_lote
# --- 6. Carga de la Arquitectura y los Pesos ---
print("\nCreando arquitectura del modelo...")
modelo = crear_arquitectura_efficientnetb0(NUM_BANDAS)
print("Cargando pesos entrenados...")
modelo.load_weights(ruta_pesos_h5)
...
# --- 8. Resultados Finales de la Validación ---
